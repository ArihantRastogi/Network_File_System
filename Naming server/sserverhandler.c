...
int send_pack_to_sserver(int socket, clientrequest *a){
    ...
    else if(x>0&&a->ping==2){
        printf("%d\n",a->ping);
        logs(1, 0, a->port, a->request, "Response received from storage server", FILE_NOT_CREATED);
        return 0;
    }
    else if(x>0&&a->ping==3){
        printf("%d\n",a->ping);
        logs(1, 0, a->port, a->request, "Response received from storage server", FILE_NOT_DELETED);
        return 0;
    }
