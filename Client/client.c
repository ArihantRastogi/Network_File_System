...
pid_t mpv_pid = -1;
#define BACK_CHANNEL_PORT 9005 // thi is the unique port on which the client will be listening for any async acks
// 192.168.203.119 8080
// known function
void man()
{
...
void handle_kind_3_request(int client_socket_fd) {
    char buffer[25000] = {'\0'};
    char accumulated_buffer[50000] = {'\0'};
    buffer[0] = '\0';
    int bytes_received;
    ...
    while ((bytes_received = recv(client_socket_fd, buffer, sizeof(buffer), 0)) > 0) {
            ...
            strcat(accumulated_buffer, buffer);
            printf("%s", buffer);
        fflush(stdout); 

        if (strstr(accumulated_buffer, "No More files") != NULL) { 
            // printf("cleared\n");
            break;
        }
    ...
    printf("Finished writing to file.\n");
        ...
        while (line != NULL) {
            ...
            if (line[0] == '-' || line[0] == 'd' || line[0] == 'l') {
                ...
                int matched = sscanf(line, "%11s %d %49s %49s %lld %3s %d %5s %255[^\n]",
